...
import { MdOutlineHome, MdOutlineSubscriptions, MdContactSupport } from "react-icons/md";
...
const Sidebar: React.FC<SidebarProps> = ({ subscribedChannels }) => {
  ...
  return (
    <>
      ...
      <div
        className={`fixed w-64 bg-purple-950 z-40 mt-2 px-6 flex flex-col h-screen overflow-scroll no-scrollbar ${
      ...
      >
        ...
        <div className="pt-6 pb-3 border-b border-b-neutral-700">
          ...
          <MenuItem
            label="Contact Us"
            logo={<MdContactSupport className="h-6 w-6 mr-4" />}
            // lable="Home"
            round
            onClick={() => handleItemClick(() => router.push("/contactUs"))}
          />
          {currentUser ? (
            <MenuItem
            label="Subscription"
